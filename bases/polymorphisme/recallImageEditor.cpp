...
public:
    ...
    {
        return abs(this->top_left.GetX() - this->bottom_right.GetX()) *
               abs(this->top_left.GetY() - this->bottom_right.GetY()); // just compute
    }
...
public:
    ...
    virtual int ComputeArea() const override
    {
        return M1_PI * pow(this->radius, 2); // just compute
    }
    virtual void Draw() const override
    {
        Shape::Draw();
